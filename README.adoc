...
Because the [hotspot=3 file=1]`io_openliberty_guides_inventory_inMaintenance` configuration property is set to `false` by default, the [hotspot=69-85]`testPutServiceInMaintenance()` test case first checks that the inventory service is not in maintenance in the beginning.
