...
The [hotspot=52-67 file=0]`testInitialServiceStatus()` test case reads the value of the [hotspot=3 file=1]`io_openliberty_guides_inventory_inMaintenance` configuration property in the file [hotspot1-5 file=1]`META-INF/microprofile-config.properties` and checks the HTTP response of the inventory service.
