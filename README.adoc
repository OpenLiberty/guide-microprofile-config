...
Inject the [hotspot=13 file=3]`io_openliberty_guides_port_number` property, and add the [hotspot=16-18 file=3]`getPortNumber()` class method to the `InventoryConfig.java` file.
...
----
inject the `io_openliberty_guides_inventory_inMaintenance` property, and add the [hotspot=24-26]`isInMaintenance()` class method. Simply copy all the code and replace the contents of the file.

The [hotspot=16]`@Inject` and [hotspot=17]`@ConfigProperty` annotations inject the `io_openliberty_guides_inventory_inMaintenance` configuration property from the `CustomConfigSource.json` file.
...
Every time that you invoke the [hotspot=25]`inMaintenance.get()` method, the `Provider<>` interface picks up the
...
To determine whether the inventory service is in maintenance or not (according to the configuration value), [hotspot=55]`inventoryConfig.isInMaintenance()` class method is used.
...
The email configuration value can be get by calling [hotspot=60]`inventoryConfig.getEmail()` method.
