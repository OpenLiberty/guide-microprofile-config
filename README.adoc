...
* `http://localhost:9080/config/manager/all`
...
Access the partially implemented `ConfigResource` Java class in the `start/src/main/java/io/openliberty/guides/config` directory.

Open the `ConfigResource.java` file and add the following configuration injection:

[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/config/ConfigResource.java[tags=port-number]
----
...
[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/config/ConfigResource.java[tags=getPortNumber]
----

Open the `InventoryResource.java` file. Inject the `ConfigResource` object to modify the existed class.
[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryResource.java[tags=config-injection]
----

Instead of hardcoding `9080` for the port, get the port number from the configuration.
[source, java, indent=0]
----
...
Access the partially implemented `ConfigResource.java` Java class in the `start/src/main/java/io/openliberty/guides/config` directory.

Add the following two configuration injections:

[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/config/ConfigResource.java[tags=config]
----
...
[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/config/ConfigResource.java[tags=build-in-converter]
----
...
Add the `isInvInMaintenance()` class method as shown in the following example:
[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/config/ConfigResource.java[tags=isInvInMaintenance]
----

Every time that you invoke the `invInMaintenance.get()` method, the `Provider<>` interface picks up the
latest value of the `io_openliberty_guides_inventory_inMaintenance` property from configuration sources.

Open the `InventoryResource.java` file. Use the `configResource.isInvInMaintenance()` class method to determine whether the inventory service is in maintenance
...
To use the custom `Email` converter, open the `ConfigResource.java` file, inject the `io_openliberty_guides_email` property, and add the `getEmail()` method:
[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/config/ConfigResource.java[tags=custom-converter;getEmail]
----

Open the `JsonMessages.java` file. Add the `returnMessage()` method.
[source, java, indent=0]
----
