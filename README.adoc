...
* The [hotspot=1-5]`META-INF/microprofile-config.properties` configuration property file on the classpath has a default ordinal of 100.
...
Inject the [hotspot=13 file=3]`io_openliberty_guides_port_number` property, and add the [hotspot=16-18 file=3]`getPortNumber()` class method to the [hotspot=1-20 file=3]`InventoryConfig.java` file.
...
A [hotspot=1-7 file=2]`CustomConfigSource.json` JSON file has already been created in the `resources` directory. This JSON file simulates a remote configuration resource in real life.
This file contains 4 custom config properties and has an ordinal of [hotspot=2 file=2]`150`.
...
[role="code_command hotspot=21-96", subs="quotes"]
----
...
The [hotspot=46-77 file=0]`getProperties()` method reads the key value pairs from the `resources/CustomConfigSource.json` JSON file and writes the information into a map.

Last but important, register the custom configuration source. 

[role="code_command hotspot=1 file=1", subs="quotes"]
----
...
[role="code_command hotspot=10-28", subs="quotes"]
----
...
The [hotspot=16 file=0]`@Inject` and [hotspot=17 file=0]`@ConfigProperty` annotations inject the [hotspot=4 file=1]`io_openliberty_guides_inventory_inMaintenance` configuration property from the [hotspot=1-7 file=1]`CustomConfigSource.json` file.
...
The [hotspot=2 file=0]`config_ordinal` value of the custom configuration source is set to `150`. It overrides configuration values of the default [hotspot=1-5 file=1]`microprofile-config.properties` source, which has a [hotspot=1 file=1]`config_ordinal` value of `100`.
...
Play with this application by changing configuration values for each property in the [hotspot=1-7 file=0]`resources/CustomConfigSource.json` file.
...
For example, change [hotspot=4 file=0]`io_openliberty_guides_inventory_inMaintenance` from `false` to `true`, then try to access http://localhost:9080/inventory/systems again.
...
The [hotspot=52-67 file=0]`testInitialServiceStatus()` test case reads the value of the [hotspot=3 file=1]`io_openliberty_guides_inventory_inMaintenance` configuration property in the file [hotspot=1-5 file=1]`META-INF/microprofile-config.properties` and checks the HTTP response of the inventory service.
...
Because the [hotspot=3 file=1]`io_openliberty_guides_inventory_inMaintenance` configuration property is set to `false` by default, the [hotspot=69-85 file=0]`testPutServiceInMaintenance()` test case first checks that the inventory service is not in maintenance in the beginning.
Next, this test switches the value of the [hotspot=3 file=1]`io_openliberty_guides_inventory_inMaintenance` configuration property to `true`.
...
To see whether the tests detect a failure, remove the configuration resetting line in the [hotspot=30-37 file=0]`setup()` method of the [hotspot=1-107 file=0]`ConfigurationTest.java` file.
Then manually change some configuration values in the [hotspot=1-7 file=2]`resources/CustomConfigSource.json` file.
