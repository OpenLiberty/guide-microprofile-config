...
Every time that you invoke the [hotspot=inMaintenanceGet file=0s]`inMaintenance.get()` method,
the `Provider<>` interface picks up the latest value of the [hotspot=4 file=1]`io_openliberty_guides_inventory_inMaintenance` property from configuration sources.
