...
Open the `InventoryConfig.java` file and add the following two configuration injections:
...
The second `@Inject` annotation injects the `io_openliberty_guides_inventory_inMaintenance` configuration property, a dynamic injection that uses the `Provider<>` interface, which forces the program to retrieve the `inMaintenance` value just in time.
...
or not according to the configuration. If you set the `io_openliberty_guides_inventory_inMaintenance` property to `true` by configuration, the inventory service returns the message, `Service is temporarily down for maintenance`.
...
{   "config_ordinal": 500,
    ...
    "io_openliberty_guides_email": "developer@OpenLiberty.io",
