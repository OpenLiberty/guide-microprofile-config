...
Access the local `microprofile-config.properties` configuration file in the `src/main/resources/META-INF` directory. This configuration file is the default configuration source for an application that uses MicroProfile Config.
...
Begin by navigating to the `pom.xml` file to check the required dependency. The `microprofile-config-api` dependency has been added for you in the `pom.xml` file.
The `mpConfig-1.1` feature has also been enabled in the `src/main/liberty/config/server.xml` file.

Now navigate to the local  `microprofile-config.properties` configuration file in the `src/main/resources/META-INF` directory to check some static configuration.
...
A `CustomConfigSource.json` JSON file has already been created outside of the application (`guide-microprofile-config/resources/CustomConfigSource.json`).
...
To link this JSON file to your application, create a `src/main/java/io/openliberty/guides/config/CustomConfigSource.java` file and add the following content to override the `ConfigSource` interface:
...
The `getProperties()` method reads the key value pairs from the `guide-microprofile-config/resources/CustomConfigSource.json` JSON file and writes the information into a map.
...
To enable this dynamic injection, first access the partially implemented `InventoryConfig.java` Java class in the `src/main/java/io/openliberty/guides/inventory` directory,
...
Create a `src/main/java/io/openliberty/guides/config/CustomEmailConverter.java` file, and add the following content to override the `Converter<T>` interface:
...
To use the custom `Email` converter, open the `InventoryConfig.java` file, inject the `io_openliberty_guides_email` property, and add the `getEmail()` method. Simply copy all the following code and replace the contents of the file:
[source, java, indent=0]
----
