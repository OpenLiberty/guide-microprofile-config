...
* `http://localhost:9080/config/manager`
...
Access the partially implemented `InventoryConfig` Java class in the `start/src/main/java/io/openliberty/guides/inventory` directory.
...
the `testOverrideConfigProperty()` test case first checks that the `http://localhost:9080/config/manager` microservice contains
