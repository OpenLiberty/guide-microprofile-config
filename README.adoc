...
Begin by opening the `pom.xml` file to check the required dependency. The `microprofile-config-api` dependency has been added for you in the `pom.xml` file.
...
A `CustomConfigSource.json` JSON file has already been created outside of the `start` directory (`guide-microprofile-config/resources/CustomConfigSource.json`).
...
To link this JSON file to your application, create a `src/main/java/io/openliberty/guides/config/CustomConfigSource.java` file in the `start` directory. Add the following content to implement the `ConfigSource` interface:
...
However, you can manually change the `config_ordinal` value to a larger number in the `guide-microprofile-config/resources/CustomConfigSource.json` file.
...
Create a `src/test/java/it/io/openliberty/guides/config/ConfigurationTest.java` file in the `start` directory and add the following code:
