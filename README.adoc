...
Open the `InventoryResource.java` file. Inject the `InventoryConfig` object to modify the existing class. Modify the `getPropertiesForHost()` service. Get the port number from the configuration and pass this value to the `manager.get()` method to get the system properties.
