...
MicroProfile Config has built-in converters that automatically converts configured Strings into target types such as `int`, `boolean`, `float` and `double`. Therefore, it is safe to directly set the variable type to `Provider<Boolean>`.
