...
Lastly, it is an important step to register the custom configuration source, open the `start/src/main/resources/META-INF/services/org.eclipse.microprofile.config.spi.ConfigSource` file and add the full class name into it:
...
To register your implementation, open the `start/src/main/resources/META-INF/services/org.eclipse.microprofile.config.spi.Converter` file and add the fully qualified class name of the custom converter into it:
[source, java, indent=0]
----
