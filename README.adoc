...
= Advanced Usages of MicroProfile Configuration

Learn how to use the MicroProfile Config specification to build an application. MicroProfile Config enables to externalize configurations from microservices.
...
You will externalize configuration data for an application, enable dynamic configuration injection,
and assign priorities to different configuration sources.
In addition, you will define a custom configuration source that is more flexible and can override configuration values for the application.
...
The `io.openliberty.guides.microprofile.inventory.inMaintenance` configuration property determines whether the inventory service is in maintenance or not.
...
The `io.openliberty.guides.microprofile.port` property determines the port number of the deployed application.
[source, java, indent=0, role="no_copy"]
----
...
The second `@Inject` annotation injects the `io.openliberty.guides.microprofile.inventory.inMaintenance` configuration property, a dynamic injection that uses the `Provider<>` interface, which forces the program to retrieve the `inMaintenance` value just in time.
...
add custom converters by implementing the generic interface `org.eclipse.microprofile.config.spi.Converter<T>`.

Open the custom converter `start/src/main/java/io/openliberty/guides/config/CustomEmailConverter.java` file.
Add the following content to override the `Converter<T>` class:
...
To register the custom converter, add the full class name in the `META-INF/services/org.eclipse.microprofile.config.spi.Converter` file:
[source, java, indent=0]
----
include::finish/src/main/resources/META-INF/services/org.eclipse.microprofile.config.spi.Converter[tags=register]
----

To use the custom `Email` converter, open the `InventoryConfig.java` file, inject the property, and add the `getEmail()` method:
[source, java, indent=0]
----
...
For example, change `io.openliberty.guides.microprofile.inventory.inMaintenance` from `false` to `true`, then try to access `http://localhost:9080/inventory/hosts` again,
...
the `io.openliberty.guides.microprofile.testConfigOverwrite` test property with the `DefaultSource` value, which is set by this default file.
...
```
{
    ...
    "io.openliberty.guides.microprofile.inventory.testCustomConfigProperty": "developer@OpenLiberty.io",
    "io.openliberty.guides.microprofile.testConfigOverwrite": "CustomSource"
}
