...
MicroProfile Config combines configuration values from multiple sources, each are known as a _ConfigSource_. Each ConfigSource has a specified priority, which is defined by its _config_ordinal_ value.
...
In the starting repo for this guide a `CustomConfigSource.json` JSON file has already been created outside of the application (`start/resource/`).
This file contains 4 custom config properties and has an ordinal of `500`.
