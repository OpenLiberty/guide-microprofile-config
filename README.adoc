...
----
include::finish/src/main/resources/META-INF/microprofile-config.properties[tags=**]
----
...
The MicroProfile Config API was added as a dependency to your [hotspot=1-252 file=0]`pom.xml` file. Look for the dependency with the [hotspot=111 file=0]`mpConfig` artifact ID.
This feature allows you to use the MicroProfile Config API to externalize configurations for your microservices.
The [hotspot=9 file=1]`mpConfig` feature is also enabled in the [hotspot=1-17 file=1]`src/main/liberty/config/server.xml` file.
...
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryConfig.java[tags=**;!copyrights]
----

Inject the [hotspot=13 file=3]`io_openliberty_guides_port_number` property, and add the [hotspot=16-18 file=3]`getPortNumber()` class method to the [hotspot=10-35 file=3]`InventoryConfig.java` file.
...
[source, Java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/config/CustomConfigSource.java[tags=**]
----

The [hotspot=45-76 file=1]`getProperties()` method reads the key value pairs from the `resources/CustomConfigSource.json` JSON file and writes the information into a map.
...
[role="code_command hotspot=9-35", subs="quotes"]
----
...
----
inject the `io_openliberty_guides_inventory_inMaintenance` property, and add the `isInMaintenance()` class method. Simply copy all the code and replace the contents of the file.
...
[role="code_command hotspot=2-25", subs="quotes"]
---- 
...
To use this [hotspot=2-25 file=0]`Email` class type, add a custom converter by implementing the generic interface `org.eclipse.microprofile.config.spi.Converter<T>`.
...
[source, Java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/config/CustomEmailConverter.java[tags=**]
----
...
[role="code_command hotspot=2-35", subs="quotes"]
---- 
...
[source, Java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryConfig.java[tags=**]
----
Inject the `io_openliberty_guides_email` property, and add the `getEmail()` method. Simply copy all the following code and replace the contents of the file.
...
[role="code_command hotspot=9-45", subs="quotes"]
----
...
[source, Java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryResource.java[tags=**]
----
...
Play with this application by changing configuration values for each property in the [hotspot=1-5]`resources/CustomConfigSource.json` file.
...
For example, change [hotspot=3]`io_openliberty_guides_inventory_inMaintenance` from `false` to `true`, then try to access http://localhost:9080/inventory/systems again.
...
[source, Java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/config/ConfigurationTest.java[tags=**]
----
