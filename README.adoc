...
[role="code_command hotspot=10-20", subs="quotes"]
----
...
[role="code_command hotspot=6-24", subs="quotes"]
----
...
[role="code_command hotspot=19-65", subs="quotes"]
----
...
The port number from the configuration is retrieved by the [hotspot=33]`inventoryConfig.getPortNumber()` method and passed to the [hotspot=33]`manager.get()` method as a parameter.

To determine whether the inventory service is in maintenance or not (according to the configuration value), [hotspot=54]`inventoryConfig.isInMaintenance()` class method is used.
...
The email configuration value can be get by calling [hotspot=59]`inventoryConfig.getEmail()` method.
...
[role="code_command hotspot=14-107", subs="quotes"]
----
...
The [hotspot=87-105]`testChangeEmail()` test case first puts the `inventory` service in maintenance, then it changes the email address in the configuration file. In the end, the `inventory` service should display the error message with the latest email address.
