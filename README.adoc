...
The MicroProfile Config API was added as a dependency to your [hotspot file=0]`pom.xml` file. Look for the dependency with the [hotspot=config_aID file=0]`mpConfig` artifact ID.
...
[source, Java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/config/Email.java[][tags=**;!copyright;]
----
...
To see whether the tests detect a failure, remove the configuration resetting line in the [hotspot=setup file=0]`setup()` method of the [hotspot file=0]`ConfigurationTest.java` file.
