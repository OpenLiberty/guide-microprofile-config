...
Access the [hotspot=1-5]`src/main/resources/META-INF/microprofile-config.properties` local configuration file. This configuration file is the default configuration source for an application that uses MicroProfile Config.
...
The MicroProfile Config API was added as a dependency to your [hotspot=1-252 file=0]`pom.xml` file. Look for the dependency with the [hotspot=108 file=0]`mpConfig` artifact ID.
This feature allows you to use the MicroProfile Config API to externalize configurations for your microservices.
The [hotspot=8 file=1]`mpConfig` feature is also enabled in the [hotspot=1-17 file=1]`src/main/liberty/config/server.xml` file.
...
Now navigate to the [hotspot=1-5 file=2]`src/main/resources/META-INF/microprofile-config.properties` local configuration file to check some static configuration.
...
Inject the [hotspot=13 file=3]`io_openliberty_guides_port_number` property, and add the [hotspot=24-26 file=3]`getPortNumber()` class method to the [hotspot=10-35 file=3]`InventoryConfig.java` file.
...
The [hotspot=24-26 file=3]`getPortNumber()` method directly returns the value of `portNumber` because it has been injected.
...
A [hotspot=1-7 file=0]`CustomConfigSource.json` JSON file has already been created in the `resources` directory. This JSON file simulates a remote configuration resource in real life.
...
[role="code_command hotspot=20-95", subs="quotes"]
----
...
[role="code_command hotspot=10-35", subs="quotes"]
----
Edit the `InventoryConfig.java` class.
`src/main/java/io/openliberty/guides/inventory/InventoryConfig.java`
----
InventoryConfig.java
[source, Java, linenums, role='code_column']
...
----
inject the `io_openliberty_guides_inventory_inMaintenance` property, and add the [hotspot=31-33]`isInMaintenance()` class method. Simply copy all the code and replace the contents of the file.

The [hotspot=12]`@Inject` and [hotspot=13]`@ConfigProperty` annotations inject the `io_openliberty_guides_inventory_inMaintenance` configuration property from the `CustomConfigSource.json` file.
...
Every time that you invoke the [hotspot=32]`inMaintenance.get()` method, the `Provider<>` interface picks up the
...
[role="code_command hotspot=3-26", subs="quotes"]
---- 
...
[role="code_command hotspot=1-24", subs="quotes"]
----
...
[role="code_command hotspot=1-35", subs="quotes"]
---- 
Edit the `InventoryConfig` class.
`src/main/java/io/openliberty/guides/inventory/InventoryConfig.java`
----
InventoryConfig.java
[source, Java, linenums, role='code_column']
...
[role="code_command hotspot=19-51", subs="quotes"]
----
...
The port number from the configuration is retrieved by the [hotspot=36]`inventoryConfig.getPortNumber()` method and passed to the [hotspot=36]`manager.get()` method as a parameter.

To determine whether the inventory service is in maintenance or not (according to the configuration value), [hotspot=61]`inventoryConfig.isInMaintenance()` class method is used.
...
The email configuration value can be get by calling [hotspot=66]`inventoryConfig.getEmail()` method.
...
The [hotspot=2]`config_ordinal` value of the custom configuration source is set to `150`. It overrides configuration values of the default `microprofile-config.properties` source, which has a `config_ordinal` value of `100`.
...
Play with this application by changing configuration values for each property in the [hotspot=1-7]`resources/CustomConfigSource.json` file.
...
For example, change [hotspot=4]`io_openliberty_guides_inventory_inMaintenance` from `false` to `true`, then try to access http://localhost:9080/inventory/systems again.
...
[role="code_command hotspot=14-106", subs="quotes"]
----
...
[source, Java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/config/ConfigurationTest.java[tags=**;!copyrights]
----

The [hotspot=52-67]`testInitialServiceStatus()` test case reads the value of the `io_openliberty_guides_inventory_inMaintenance` configuration property in the file `META-INF/microprofile-config.properties` and checks the HTTP response of the inventory service.
...
Because the `io_openliberty_guides_inventory_inMaintenance` configuration property is set to `false` by default, the [hotspot=69-85]`testPutServiceInMaintenance()` test case first checks that the inventory service is not in maintenance in the beginning.
...
The [hotspot=87-104]`testChangeEmail()` test case first puts the `inventory` service in maintenance, then it changes the email address in the configuration file. In the end, the `inventory` service should display the error message with the latest email address.
...
To see whether the tests detect a failure, remove the configuration resetting line in the [hotspot=30-37]`setup()` method of the `ConfigurationTest.java` file.
