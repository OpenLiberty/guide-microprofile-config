...
Last but important, register the custom configuration source. Create a `start/src/main/resources/META-INF/services/org.eclipse.microprofile.config.spi.ConfigSource` file. Add the following fully qualified class name of the configuration source into it:
...
inject the `io_openliberty_guides_inventory_inMaintenance` property, and add the `isInMaintenance()` class method. A simple way to proceed is to copy all the following code and replace the content of the file:
...
To register your implementation, create a `start/src/main/resources/META-INF/services/org.eclipse.microprofile.config.spi.Converter` file. Add the following fully qualified class name of the custom converter into it:
[source, java, indent=0]
----
include::finish/src/main/resources/META-INF/services/org.eclipse.microprofile.config.spi.Converter[tags=register]
----

To use the custom `Email` converter, open the `InventoryConfig.java` file, inject the `io_openliberty_guides_email` property, and add the `getEmail()` method. A simple way to proceed is to copy all the following code and replace the content of the file:
[source, java, indent=0]
----
...
A simple way to proceed is to copy all the following code and replace the content of the file:
[source, java, indent=0]
----
...
Reminder that you must register the custom configuration source and custom converter in the `start/src/main/resources/META-INF/services/` directory, if you miss to complete these steps, the tests will fail.
