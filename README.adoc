...
You will learn how to externalize configuration data for an application, inject Config into microservices,
enable dynamic configuration injection, and assign priorities to different configuration sources.
...
The `io_openliberty_guides_port_number` property determines the port number of the REST service.
[source, java, indent=0, role="no_copy"]
----
include::finish/src/main/resources/META-INF/microprofile-config.properties[tags=inventory-port-number]
----

Create an `InventoryConfig.java` file in the `start/src/main/java/io/openliberty/guides/inventory` directory.
...
or not according to the configuration. If you set the `io_openliberty_guides_inventory_inMaintenance` property to `true` in the configuration, the inventory service returns the message, "Service is temporarily down for maintenance", along with the contact email.
Get the email configuration value by `inventoryConfig.getEmail()` and pass this value to the `returnMessage()` method.
